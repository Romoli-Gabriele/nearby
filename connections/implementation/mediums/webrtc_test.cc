...
TEST_F(WebRtcTest, CancelDuringConnect) {
  ...
  EXPECT_FALSE(sender_socket.IsValid());
  EXPECT_EQ(1, sender->num_connect_attempts_count());
...
TEST_F(WebRtcTest, CancelBeforeConnect) {
  ...
  // Expect an invalid socket and no attempts to connect, because `Connect`
  // returned immediatley when it checked for cancellation.
  EXPECT_FALSE(sender_socket.IsValid());
  EXPECT_EQ(0, sender->num_connect_attempts_count());
