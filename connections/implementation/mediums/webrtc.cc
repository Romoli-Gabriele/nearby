...
WebRtcSocketWrapper WebRtc::Connect(const std::string& service_id,
  ...
  if (cancellation_flag->Cancelled()) {
    NEARBY_LOGS(WARNING) << "Cannot start Connect with WebRtc due to cancel.";
    return WebRtcSocketWrapper();
  }

  for (num_connect_attempts_count_ = 1;
       num_connect_attempts_count_ <= kConnectAttemptsLimit;
       num_connect_attempts_count_++) {
    ...
    if (cancellation_flag->Cancelled()) {
      NEARBY_LOGS(WARNING) << "Cannot connect with WebRtc due to cancel.";
      return WebRtcSocketWrapper();
    }
  }
...
WebRtcSocketWrapper WebRtc::AttemptToConnect(
  ...
  CancellationFlagListener listener(
      cancellation_flag, [&socket_future]() {
        NEARBY_LOGS(WARNING)
            << "Attempt to connect with WebRtc stopped due to cancel.";
